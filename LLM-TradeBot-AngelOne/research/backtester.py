...
class Backtester:
    ...
    def run_backtest(self, strategy_func, symbol="BTCUSDT", interval="1h", 
        ...
        print(f"\n{'='*60}")
        print(f"Starting backtest: {symbol} ({interval}), {days} days")
        print(f"{'='*60}")
        ...
        for i, kline in enumerate(klines):
            ...
            if signal == 'BUY' and position == 0:
                ...
                print(f"[{current_time}] BUY @ ${current_price:.2f}")
                ...
                print(f"[{current_time}] SELL @ ${current_price:.2f} | P&L: ${profit:.2f} ({profit_pct:+.2f}%)")
                position = 0
