...
Specification:
- Data: Last 10 bars of 5min candlesticks
- Volume MA3: Average of last 3 bars
- Engulfing: Previous bearish + Current bullish engulfs previous
...
class TriggerDetector:
    ...
    Implements specification Module 4: Trigger Module
    """
    ...
    def detect_engulfing(self, df_5m: pd.DataFrame, direction: str = 'long') -> Dict:
        """
        Detect Engulfing pattern (Bullish engulfing for long, Bearish engulfing for short)
        ...
        if direction == 'long':
            # Previous is bearish candle
            prev_bearish = prev['close'] < prev['open']
            # Current is bullish candle
            curr_bullish = curr['close'] > curr['open']
            # Bullish engulfs bearish
            engulfing = curr['close'] > prev['open'] and curr['open'] < prev['close']
            
            detected = prev_bearish and curr_bullish and engulfing
            
        else:  # short
            # Previous is bullish candle
            prev_bullish = prev['close'] > prev['open']
            # Current is bearish candle
            curr_bearish = curr['close'] < curr['open']
            # Bearish engulfs bullish
            engulfing = curr['close'] < prev['open'] and curr['open'] > prev['close']
    ...
    def calculate_rvol(self, df: pd.DataFrame, lookback: int = 10) -> float:
        ...
        Args:
            df: Candlestick data with 'volume' column
