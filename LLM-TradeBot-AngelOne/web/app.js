...
function updateLanguageButton() {
    ...
    if (langText) {
        langText.textContent = window.currentLang === 'en' ? 'ZH' : 'EN';
    }
}
...
// Global storage for decision history for filtering
...
function renderDecisionTable(history, positions = []) {
    ...
    tbody.innerHTML = history.map(d => {
        // Only show hours:minutes:seconds (HH:MM:SS)
        ...
        .then(res => {
            if (res.status === 403) {
                ...
                return res.json().then(data => {
                    alert(data.detail || 'Demo time has expired');
...
function showAccountAlert(failureCount) {
    ...
    content.innerHTML = `
        <div style="text-align: center;">
            ...
            <h2 style="color: #ff4444; margin: 0 0 15px 0; font-size: 24px;">Account Information Fetch Failed</h2>
            <p style="color: #94a3b8; margin: 0 0 10px 0; line-height: 1.6;">
                Unable to fetch account information for <strong style="color: #ff4444;">5 minutes</strong> continuously
            </p>
            <p style="color: #64748b; margin: 0 0 25px 0; font-size: 14px;">
                Consecutive failures: <span style="color: #ff4444; font-weight: bold;">${failureCount}</span>
            </p>
